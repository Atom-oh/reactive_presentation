...
<div class="flow-v" style="gap:0.6rem; padding:0.3rem;">
  ...
  <div class="flow-h" style="justify-content:center; gap:1.5rem;">
    <div class="flow-box bg-dark" style="min-width:140px; padding:0.8rem;" data-fragment-index="1">
      ...
      <span style="font-size:0.9rem;">mall.example.com</span>
    </div>
  ...
  <div class="flow-h" style="justify-content:center; gap:2rem;">
    ...
    <div class="flow-box bg-purple" style="min-width:200px; padding:0.7rem;" data-fragment-index="5">
      ...
      <div class="flow-h" style="gap:0.5rem; align-items:center;">
        ...
        <span style="font-size:0.8rem;">api-internal.example.com</span>
      </div>
...
사용자가 mall.example.com으로 접속하면, Route53이 CNAME 레코드로 CloudFront 도메인을 반환합니다. 이 첫 번째 DNS 조회가 글로벌 트래픽 분산의 시작점입니다.
...
{cue: pause}
/api/* 경로는 api-internal.example.com으로 라우팅됩니다. 여기서 두 번째 Route53 조회가 발생하는데, 이번에는 Latency-based Routing으로 사용자에게 가장 가까운 리전의 NLB IP를 반환합니다.
...
DNS 레코드 구조를 보면, mall.example.com은 CloudFront를 가리키는 CNAME이고, api-internal.example.com은 Latency-based A 레코드로 두 리전의 NLB를 가리킵니다.
...
<div class="col-2" style="gap:1.5rem; align-items:start;">
  ...
  <div class="card" style="padding:1rem;" data-fragment-index="1">
    ...
    <div style="font-family:monospace; font-size:0.85rem; background:rgba(0,0,0,0.3); padding:0.6rem; border-radius:4px; margin-bottom:0.8rem;">
      <span style="color:#00d68f;">mall.example.com</span>
    </div>
  ...
  <div class="card" style="padding:1rem;" data-fragment-index="2">
    ...
    <div style="font-family:monospace; font-size:0.85rem; background:rgba(0,0,0,0.3); padding:0.6rem; border-radius:4px; margin-bottom:0.8rem;">
      <span style="color:#00d68f;">api-internal.example.com</span>
    </div>
    <div style="font-size:0.85rem; color:var(--text-secondary); line-height:1.6;">
...
Public DNS인 mall.example.com은 CloudFront로 CNAME되어 있어 별도 Health Check가 필요 없습니다. CloudFront가 자체적으로 Origin Health를 관리하기 때문입니다.

핵심은 api-internal.example.com입니다. Latency-based A 레코드로 두 리전의 NLB IP를 가리키며, 각 리전에 대해 독립적인 Health Check가 동작합니다.
...
```yaml
Path Pattern: /api/*
Origin: api-internal.example.com (NLB)
  ...
  <div class="card" style="padding:1rem; border-left:3px solid var(--accent);" data-fragment-index="1">
    ...
    <div class="flow-h" style="gap:2rem; justify-content:center;">
      <div class="flow-box bg-green" style="min-width:180px; padding:0.8rem;">
        ...
        <div style="font-weight:600;">api-internal.example.com</div>
      ...
      <div class="flow-box bg-orange" style="min-width:180px; padding:0.8rem;">
        ...
        <div style="font-weight:600;">api-secondary.example.com</div>
...
현재 설정에서 Primary는 api-internal.example.com으로, Route53 Latency-based Routing을 통해 가장 가까운 NLB로 라우팅됩니다. Secondary는 api-secondary.example.com으로, us-west-2 NLB를 직접 가리킵니다.
